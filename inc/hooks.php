...
function kiemviet_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Xem thêm', 'kiemviet' ) . '...</a>';
}
...
function kiemviet_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Xem thêm...', 'kiemviet' ) );
}
...
function woocommerce_custom_product_add_to_cart_text() {
	return __( 'Thêm vào giỏ hàng', 'kiemviet' );
}

function kiemviet_custom_change_cart_string( $translated_text, $text, $domain ) {
	$translated_text = str_replace( 'view cart', 'Giỏ hàng', $translated_text );
	$translated_text = str_replace( 'View cart', 'Giỏ hàng', $translated_text );
